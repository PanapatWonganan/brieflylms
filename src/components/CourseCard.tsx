...
export function CourseCard({ course, index = 0 }: CourseCardProps) {
  ...
  return (
    <motion.div
      ...
      className="bg-white rounded-2xl shadow-lg overflow-hidden hover:shadow-xl transition-all duration-300 group border border-gray-200 w-full max-w-sm mx-auto sm:max-w-none"
    >
      <div className="relative h-48 bg-gradient-to-br from-blue-600 to-indigo-600 overflow-hidden">
        ...
        >
          <BookOpen
            className={`h-5 w-5 ${
              isFavorited ? "text-blue-500 fill-current" : "text-white"
            }`}
          />
        </button>
      ...
      <div className="p-4 sm:p-6">
        <h3 className="text-lg sm:text-xl font-semibold text-gray-700 mb-2 line-clamp-2 group-hover:text-blue-600 transition-colors">
          {course.title}
        </h3>
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-0">
          ...
          <Link href={`/courses/${course.id}`} className="w-full sm:w-auto">
            <motion.button
              ...
              whileTap={{ scale: 0.95 }}
              className="w-full sm:w-auto bg-gradient-to-r from-blue-500 to-indigo-500 text-white px-4 sm:px-6 py-2 rounded-lg hover:from-blue-600 hover:to-indigo-600 transition-colors font-medium text-sm sm:text-base"
            >
              เริ่มเรียนเลย
            </motion.button>
          </Link>
